  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <nav className="nav-stack" aria-label="主导航">
        {(Object.keys(views) as ViewKey[]).map((key) => { const ItemIcon = views[key].icon; return <button key={key} className={`nav-item ${active === key ? "active" : ""}`} onClick={() => setActive(key)}><ItemIcon size={17}/><span>{views[key].title.replace("工作空间", "")}</span>{key === "studio" && <span className="nav-badge">3</span>}</button>; })}
      </nav>
      <div className="sidebar-bottom"><div className="system-card"><div className="system-line"><span>协同引擎</span><Activity size={14}/></div><div className="system-value"><span className="pulse-dot"/> LangGraph 正常</div></div><button className="nav-item"><Settings2 size={17}/><span>系统设置</span></button></div>
    </aside>
    <main className="main-area">
      ...
      <div className="content">{active === "dashboard" && <DashboardView onNavigate={() => setActive("studio")}/>} {active === "studio" && <StudioView notify={notify}/>} {active === "capabilities" && <CapabilitiesView notify={notify}/>} {active === "profile" && <ProfileView notify={notify}/>}</div>
    </main>
