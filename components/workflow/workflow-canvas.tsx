...
export function WorkflowCanvas({ nodes, edges, selectedId, onSelect, onNodesChange, onEdgesChange, onConnect }: Props) {
  ...
  return <ReactFlow
    ...
    onNodeClick={onNodeClick} fitView fitViewOptions={{padding:.18}} minZoom={.45} maxZoom={1.6}
    colorMode="light" deleteKeyCode={["Backspace", "Delete"]}
    defaultEdgeOptions={{style:{stroke:"#93a79b",strokeWidth:1.4},labelStyle:{fontSize:9,fill:"#6c7a72"}}}
  >
    <Background gap={20} size={1} color="#dfe7e1"/>
    <MiniMap pannable zoomable nodeColor={(node) => node.id === selectedId ? "#8bb346" : "#dfe7e1"} maskColor="rgb(244 247 243 / 72%)" style={{border:"1px solid #dfe7e1",borderRadius:10}}/>
