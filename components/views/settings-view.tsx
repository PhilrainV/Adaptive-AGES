...
const providers = [
  ...
  { value:"newapi", label:"NewAPI / OneAPI 转接", baseUrl:"", model:"gpt-4.1-mini", placeholder:"https://docs.newapi.pro/v1", help:"填写服务商文档提供的 Base URL；支持 https://docs.newapi.pro/v1 及自部署 NewAPI 地址。" },
...
export function SettingsView({notify}:{notify:(message:string)=>void}) {
  ...
  return <>
    ...
    <div className="settings-grid"><section className="panel settings-form"><div className="panel-header"><div><h3>默认模型 API</h3><p>API Key 在后端加密存储，不会返回浏览器</p></div>{settings.api_key_configured ? <span className="status-pill live"><CheckCircle2 size={12}/>已配置</span> : <span className="status-pill review">未配置</span>}</div><div className="settings-fields">
      ...
      <div className="settings-actions wide"><button className="ghost-button" onClick={() => void saveAndTest()} disabled={testing || saving}>{testing ? <LoaderCircle className="spin" size={13}/> : <PlugZap size={13}/>}保存并测试</button><button className="primary-button" onClick={() => void save()} disabled={saving}>{saving ? <LoaderCircle className="spin" size={13}/> : <Save size={13}/>}保存</button></div>
